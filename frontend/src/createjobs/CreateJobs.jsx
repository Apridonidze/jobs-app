...
const CreateJobs = ( { setToggleSeeMore } ) => {
    ...
    return(
        <div className="create-jobs-container container d-flex flex-column justifty-content-between min-vh-100 pt-5">
            ...
            {toggleCreateJobsInput && <> <div className="create-jobs-input-background bg-dark opacity-50 position-fixed w-100 h-100 top-0 start-0" style={{zIndex : 4000}} onClick={() => setToggleCreateJobsInput(false)}></div> <CreateJobsInput SubmitBtnRef={SubmitBtnRef} setIsJobsSuccessful={setIsJobsSuccessful} setToggleJobsMessage={setToggleJobsMessage} setJobsMessage={setJobsMessage} setToggleError={setToggleError}/> {toggleJobsMessage && <NewJobsMessage  isJobsSuccessful={isJobsSuccessful} jobsMessage={jobsMessage} setToggleJobsMessage={setToggleJobsMessage} setToggleCreateJobsInput={setToggleCreateJobsInput} SubmitBtnRef={SubmitBtnRef}/>} {toggleError && <Error setToggleError={setToggleError}/>} </> }
